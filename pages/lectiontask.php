...
$sql_1 = "SELECT * FROM task WHERE lectionId = $id;";
$resultat_1 = $kobling->query($sql_1);

while ($rad_1 = $resultat_1->fetch_assoc()) {
  $video = $rad_1["task_url"];
  $solution = $rad_1["solution_url"];
}
...
<body>
  ...
  <section class="main-4">
    <div class="videobox-1">
        <?php
          echo $video;
        ?>
    </div>
    <div class="videobox-2">
    <?php
          echo $solution;
        ?>
    </div>
  ...
  <nav class="navigation-2 flex">
    <ul class="navigation__list-2">
        <li class="navigation__item-2  left"><a class="link" <?php echo "href='lection.php?ID=$id'"; ?>>Video</a></li>
        ...
        <li class="navigation__item-2 active right"><a class="link active" <?php echo "href='lectiontask.php?ID=$id'"; ?>>Task</a></li>
    </ul>
    <button href="" class="btn-tertiary ">Complete</button>
  </nav>
